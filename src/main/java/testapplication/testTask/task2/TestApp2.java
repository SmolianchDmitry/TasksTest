...
public class TestApp2{
    ...
    private static final String MESSAGE_CVV_PREFIX = "Code"; // (Дмитрий)была опечатка
    ...
    private static String maskCipherCVCInPack(String xml) {
        ...
        while (startPos > 0) {
            buf = buf.substring(startPos, buf.length()); // (Дмитрий) я бы удалил "buf.length()" , т.к. это неиспользуемый аргумент
    ...
    private static String maskCipherCVCInMsg(String message) {
        ...
        if (startPos == -1) { // (Дмитрий)проверяем, что в сообщении есть CVV
            return message;   // (Дмитрий)если нет - возвращаем исходную строку
        }
        ...
        message = message.substring(0, startPos) + mask + message.substring(startPos + cipherLength, message.length());//(Дмитрий) я бы удалил "message.length()" , т.к. это неиспользуемый аргумент
        return message;
    }
